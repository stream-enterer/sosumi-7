...
6. **SP4 test-hack removal.** Delete `test_window_id: Option<WindowId>` on `emView` and the `emWindow::new_for_test` test-only substitution path. Real view identity now exists via the `Weak<RefCell<emView>>` back-reference on panels. Tests that registered under a synthetic `WindowId` migrate to real `emWindow` construction (or a test helper that produces a genuine `emWindow` + `emView` pair).
...
Delete `test_window_id: Option<WindowId>` on `emView`. Delete `emWindow::new_for_test`'s test-only identity substitution. Migrate the tests that relied on those hacks to either real `emWindow` construction or a replacement helper that produces a genuine `(emWindow, emView)` pair with a real `Rc<RefCell<emView>>`.

Gate:
- `grep -n "test_window_id\|new_for_test" crates/emcore/src/` returns zero matches.
- nextest green.
...
- `grep -n "test_window_id\|TODO(per-view-notice-dispatch)\|PanelTree::HandleNotice"` returns zero matches in `crates/`.
