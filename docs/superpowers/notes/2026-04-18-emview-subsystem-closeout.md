...
**Commits covered:** `fc57a6a`..`47efa6b` on `main`.
**Supersedes:**
...
1. ~~`materialize_popup_surface` duplicates ~50 lines of `emWindow::create()`'s wgpu surface init. A `build_materialized_surface(gpu, winit_window) -> MaterializedSurface` helper would deduplicate. Plan flagged this as optional.~~ **Closed by SP6 on 2026-04-19** (`894bdba`): extracted as `MaterializedSurface::build(gpu, winit_window)` associated function in `emWindow.rs`; both call sites migrated. No observable behaviour change.
...
- Post-SP5 (2026-04-18): **2434/2434** (net +2: Task 2.1 `panel_data_view_defaults_none` field-default test, Task 4.4 `sp5_per_view_notice_dispatch_uses_correct_pixel_tallness` multi-view test). Baseline golden 237/243 unchanged.
- Post-SP4.5 (2026-04-19): **2440/2440** (+6: `sp4_5_panel_engine_registered_at_init_panel_view`, `sp4_5_child_panel_engine_registered_via_init_propagation`, `sp4_5_panel_engine_deregistered_on_panel_removal`, `sp4_5_register_pending_engines_catches_late_scheduler_attach`, `sp4_5_panel_cycle_uses_per_view_pixel_tallness`, `sp4_5_wake_up_panel_from_cycle_reaches_sibling`). Baseline golden 237/6 unchanged.
- Post-SP8 (2026-04-19): **2443/2443** (+3: `sp8_sub_view_update_engine_registered`, `sp8_sub_tree_root_panel_engine_registered`, `sp8_cycle_drives_sub_scheduler`). Baseline golden 237/6 unchanged.
...
12. ~~**[W5b / DEFERRED] Per-view notice dispatch (emView.cpp:1312 parity)**~~ **CLOSED 2026-04-18 by SP5** (branch `sp5-per-view-notice-dispatch`, merged as `47efa6b`). Resolution: `emView::HandleNotice` is now called from `emView::Update` per-view using that view's own `CurrentPixelTallness` and `window_focused` — emView.cpp:1312 parity. Ring data structure (`notice_ring_head_next`/`notice_ring_head_prev`) stays on `PanelTree` as a forced Rust divergence (RefCell re-entrancy prevents view-owned ring — see §8.1 item 16 SP4.5 review notes for the full decision trail). Annotated at `emPanelTree.rs:~297`. `emPanel::View: Weak<RefCell<emView>>` added (SP5 Phase 2) to establish per-panel view ownership. `emView` wrapped in `Rc<RefCell<emView>>` at `emWindow` and `emSubViewPanel` (SP5 Phase 1). SP4 test hacks deleted: `test_window_id` field on `emView`, `emWindow::new_for_test`, `emView::new_for_test` (SP5 Phase 5). Also fixed a pre-existing hole in `SetFocused` and `set_active_panel` that wrote `pending_notices` via `tree.get_mut()` without enrolling panels in the ring — masked pre-SP5 by the framework global safety-net scan, exposed when that scan was removed, closed by routing both through `tree.queue_notice`. Key commits: `f53426a`, `9999eef`, `a0e882d`, `a64ab01`, `4d81961`, `a0ba42e`, `32028a6`, `542c840`, `1301a94`, `dbf8317`, `06535e5`, `8f67c0a`, `d7f4063`, `324942f`, `c8f8476`, `a41661b` (merge: `47efa6b`). Tests: 2432 → 2434 (+2: `panel_data_view_defaults_none`, `sp5_per_view_notice_dispatch_uses_correct_pixel_tallness`). `run_panel_cycles` is a separate Rust-only construct; stays open as SP4.5.
...
16. ~~**[ARCH / SP4.5] `emPanel` engine-registration port (`run_panel_cycles` Rust-only divergence)**~~ **CLOSED 2026-04-19 by SP4.5** (branch `sp4.5-empanel-engine-registration`). Resolution: Option B (full port to the scheduler). Each `emPanel` gets a `PanelCycleEngine` adapter (one forced `DIVERGED:` at `emPanelCycleEngine.rs` citing `emPanel.h:33`'s single-inheritance — Rust cannot simultaneously store `PanelBehavior` in `PanelTree::panels` and `Box<dyn emEngine>` in the scheduler's slot-map) registered with the owning view's scheduler at `PanelTree::init_panel_view` / `create_child`. `register_pending_engines()` runs as a catch-up pass after `attach_to_scheduler` for the production ordering in `emMainWindow` where panels are created before the scheduler attaches. Deregistration goes through `queue_or_apply_sched_op(SchedOp::RemoveEngine)` in `PanelTree::remove` so a panel removed from inside a sibling's Cycle defers the scheduler mutation until after the slice. `emGUIFramework::about_to_wait`'s pick-first-window pixel-tallness block was deleted — the multi-window bug is fixed by construction (each engine's Cycle reads its own view's `GetCurrentPixelTallness`). Four call-site migrations retargeted to `ctx.wake_up{,_panel}` through `PanelCtx`. **Residual:** `PanelTree::cycle_list`, `Cycle`, `cancel_cycle`, and `run_panel_cycles` were not deleted — `emSubViewPanel::Paint` still drives sub-tree settlement synchronously inside Paint via these methods (the sub-view has no scheduler attached), and the golden `tests/golden/composition.rs::settle()` helper also depends on them. That is a separate architectural divergence and was split out as new sub-project **SP8** (see §8.0 and §8.1 item 17 below). Keys: tests 2434 → 2440 (+6: Phase 2 registration/deregistration/late-attach lifecycle + Phase 5 multi-view tallness + mid-slice sibling wake); golden 237/6 unchanged.

    **Follow-up SP4.5-FIX-1 — register-path re-entrancy** (2026-04-19, post-SP7 closeout). `register_engine_for` was only symmetric with `deregister_engine_for` for the *no-engine* / *bare-view* cases; on the *scheduler busy* and *view borrow_mut held* cases it called `sched_rc.borrow_mut()` and `view_rc.borrow()` unconditionally. Production startup hit the first one at the first `StartupEngine::Cycle` → `create_child`; input dispatch hit the second through `dispatch_input` → `emView::Update` → `emVirtualCosmosPanel::update_children` → `create_child`. Both panicked the binary on launch. Fix: `register_engine_for` now uses `try_borrow` on the view and `try_borrow_mut` on the scheduler; on contention it returns silently and the post-`DoTimeSlice` `tree.register_pending_engines()` sweep (added in `App::about_to_wait` and `emSubViewPanel::Cycle`) registers the deferred panel on the way out of the slice. Mirrors SP4's `SchedOp` defer pattern. Tests 2446 → 2448 (+2 regression tests: `sp4_5_create_child_from_inside_engine_cycle_does_not_panic`, `sp4_5_create_child_with_view_already_borrow_mut_does_not_panic`); golden 237/6 unchanged. Smoke: rebuilt release binary stays alive past 20 s with no panic.

    **Follow-up SP4.5-FIX-2 — `RawVisitAbs` popup-creation re-entrancy** (open as of 2026-04-19). Surfaced by Part A of the SP4.5-FIX-1 follow-ups audit (spec `2026-04-19-sp4-5-fix-1-followups-design.md`, plan `2026-04-19-sp4-5-fix-1-followups.md`, audit row at `docs/superpowers/notes/2026-04-19-sp4-5-fix-1-audit-table.md` line `emView.rs:1832`). The popup-creation block inside `RawVisitAbs` calls `sched.borrow_mut()` to allocate four signals (`close_sig`, `flags_sig`, `focus_sig`, `geom_sig`) for a new popup `emWindow`. Reachable from `Update` → `SVPChoiceInvalid` branch → `RawVisitAbs` (the `&mut emView` callgraph), which is invoked from `UpdateEngineClass::Cycle` while `DoTimeSlice` holds `scheduler.borrow_mut()`. Re-entrant scheduler borrow would panic if the path triggers; uncommon (requires popup-zoom + outside-home + `SVPChoiceInvalid` in the same slice), so the panic is latent and not observed in current smoke testing.

    **Why SP4.5-FIX-1 template doesn't fit.** Two attempted fixes (commits `a67bdc0`, `95d7266`) using the SP4.5-FIX-1 `try_borrow_mut` + early-return pattern were reverted because:
    - The deferred work needs scheduler access *to produce* the four signal IDs that wire the popup synchronously — they cannot be allocated post-slice and pasted in.
    - The natural retry mechanism (`WakeUpUpdateEngine` queued via `pending_sched_ops`, drained by `Cycle`, next slice runs `Update`) does not preserve the `SVPChoiceInvalid` flag that triggered `RawVisitAbs`. The flag is cleared by `Update`'s loop at `emView.rs:~2506` before each `RawVisitAbs` call, so by the time the catch-up tick fires, the loop sees no work to do and `RawVisitAbs` is never re-entered. The fix would convert the panic into a silent missing popup.

    **Three candidate fix shapes** (option C from the SP4.5-FIX-1-followups brainstorming, selected for the SP4.5-FIX-2 follow-up):
    1. **Pre-allocate signals at view construction.** Move the four `create_signal()` calls from `RawVisitAbs` into `emView::new` (or `attach_to_scheduler`) so the IDs exist before `Update` ever runs. Popup creation inside `RawVisitAbs` then uses the pre-allocated IDs without scheduler access. Cleanest architecturally; modest blast radius (`emView` field additions; signal-lifecycle changes). May force a one-popup-per-view semantic (signals reused or re-allocated post-popup-teardown — needs a sub-decision).
    2. **Queue popup creation as a new `SchedOp` variant.** Add `SchedOp::CreatePopupSignals(...)` carrying a callback that, when drained post-`DoTimeSlice`, allocates signals + constructs the popup. Matches the SP4 `SchedOp` defer pattern but introduces stateful queueing of an `emWindow` constructor — risk of partial state observable mid-slice.
    3. **Re-set `SVPChoiceInvalid = true` before early-return.** Minimal hack: makes the catch-up wake actually re-enter `RawVisitAbs`. Repurposes the flag (semantic drift: it now means "SVP child invalid OR popup-creation deferred"). Stop-gap; not architecturally sound.

    **Recommended approach.** Option 1 (pre-allocate at construction). Aligns with C++ which allocates signals as part of view setup, not on-demand inside `RawVisitAbs`. Verify before committing: read C++ `emView::emView` and `emView::Visit` paths at `~/git/eaglemode-0.96.4/src/emCore/emView.cpp:~1638` (the analogous popup-creation site) and confirm whether C++ also allocates signals on-demand or pre-allocates them at view construction.

    **Blast radius estimate.** ~4 signal fields added to `emView`; `emView::new` and `attach_to_scheduler` allocate them; `RawVisitAbs` reads the pre-allocated IDs; popup teardown rotates / re-uses them. ~30–80 LOC + one regression test that drives the production `Update → SVPChoiceInvalid → RawVisitAbs → popup-creation` chain via a real `DoTimeSlice` with the contention shape preserved.
    ...
    **SP4.5-FIX-1 follow-ups closeout** (2026-04-19). Spec [`docs/superpowers/specs/2026-04-19-sp4-5-fix-1-followups-design.md`]; plan [`docs/superpowers/plans/2026-04-19-sp4-5-fix-1-followups.md`]; branch `sp4-5-fix-1-followups`.

    - **Part A (audit).** 145 rows audited across `emPanelTree`, `emPanelCtx`, `emSubViewPanel`, `emView`. Verdicts: 1 vulnerable, 0 needs-deeper-analysis (initial classification), 144 safe. The 1 vulnerable row (`emView.rs:1832`, `RawVisitAbs` popup signal allocation) was attempted via the SP4.5-FIX-1 template; both attempts (commits `a67bdc0`, `95d7266`) reverted because the natural retry mechanism doesn't preserve the `SVPChoiceInvalid` trigger flag — the fix would convert a latent panic into a silent missing popup. Row downgraded to `needs-deeper-analysis` and escalated as **SP4.5-FIX-2**. Audit table at [`docs/superpowers/notes/2026-04-19-sp4-5-fix-1-audit-table.md`].
    - **Part B (timing).** Rust baselines locked at 1 slice across all three measured paths (top-level `StartupEngine`, top-level mid-`Update`, sub-scheduler) via fixtures at commits `b4681d3`, `66decfc`, `d4238d8`. C++ measurement on Eagle Mode 0.96.4 captured delta=0 across 169 panels at startup. The +1 slice drift is filed as **SP4.5-FIX-3** with a same-slice fix shape (`SchedOp::RegisterPanelEngine` drained inside `UpdateEngineClass::Cycle` + C++ priority re-ascent). Measurement details and C++ instrumentation diff at [`docs/superpowers/notes/2026-04-19-sp4-5-fix-1-timing-measurements.md`].
    - **Tests.** 2448 → 2451 (+3 timing baseline fixtures). No production-code changes; only test-only probe infrastructure (commit `34cf771`: `EngineCtx::time_slice_counter`, `PanelCycleEngineFirstCycleProbe`, `EngineScheduler::attach_first_cycle_probe`, all `cfg(test, feature = "test-support")`).
    - **Smoke.** Release binary stays alive past 20 s, no panic. Golden 237/6 unchanged.
    - **Net outcome.** No production behavior change in this follow-up wave. Two new sub-projects filed (SP4.5-FIX-2 popup signal allocation, SP4.5-FIX-3 same-slice registration). The audit confirmed that SP4.5-FIX-1's defer pattern is the only remaining instance of its shape in the four files audited; the two filed follow-ups are about different mechanisms.
...
17. ~~**[ARCH / SP8] Sub-view synchronous-settlement divergence**~~ **CLOSED 2026-04-19 by SP8** (6 commits on main: `54ccc5d`..`2b3a186`). **Resolution:** `emSubViewPanel` gained a per-sub-view `sub_scheduler: Rc<RefCell<EngineScheduler>>` attached at construction; the sub-view's `UpdateEngine`/`VisitingVAEngine` register against it via the new view-direct engine shape (Phase 1: both engines now hold `Weak<RefCell<emView>>` instead of `WindowId`). Sub-tree panels register `PanelCycleEngine` adapters on `sub_scheduler` via `register_pending_engines`. A new `PanelBehavior::Cycle` on `emSubViewPanel` drives one `sub_scheduler.DoTimeSlice` per outer-scheduler tick and ticks `active_animator` with wall-clock dt — replacing the 50-iter synchronous settle loop inside Paint. `emSubViewPanel::Paint` now delegates to `paint_sub_tree` only, matching C++ `emSubViewPanel.cpp:94`. Golden `settle()` helper rewritten to drive `scheduler.DoTimeSlice` + explicit per-view `Update` (SP5-style). `PanelTree::cycle_list` / `Cycle` / `cancel_cycle` / `run_panel_cycles` deleted outright. One new `DIVERGED:` block at `emSubViewPanel::new`'s `sub_scheduler` field documenting the per-sub-view scheduler (forced by Rust's nested `PanelTree` ownership + singular `EngineCtx::tree`; unifiable only via SP7 `emContext` threading). Tests 2440 → 2443 (+3 SP8 tests); golden 237/6 unchanged.
    ...
    *Original item below, preserved for the historical record.*
    ...
    Surfaced 2026-04-19 during SP4.5 implementation (Phase 4 grep pass). SP4.5 planned to delete `PanelTree::cycle_list` / `Cycle` / `cancel_cycle` / `run_panel_cycles` once top-level panel cycling moved through the scheduler via `PanelCycleEngine`; the grep turned up two additional callers the original plan missed, and the user narrowed SP4.5 scope rather than expanding it.

    **Gap.** In C++ every panel (top-level or inside a sub-view) shares the scheduler through the parent `emContext`; settlement (cycling + HandleNotice + Update + animator ticks) is a cross-frame concern that falls out of the normal scheduler loop. In Rust, `emSubViewPanel` (`crates/emcore/src/emSubViewPanel.rs:303, 325`) constructs a self-contained sub-tree with no scheduler attached, and `SubViewPanel::Paint` runs a 50-iteration "settle loop" inside the Paint call that interleaves `PanelTree::run_panel_cycles`, `emView::HandleNotice`, `emView::Update`, and animator ticks. Because the sub-view has no scheduler, SP4.5 could not migrate this path to `PanelCycleEngine` without also deciding how sub-views acquire a scheduler (and re-capturing golden baselines if single-frame snapshots change). The golden `tests/golden/composition.rs::settle()` helper (~4 call sites) depends on the same API shape for deterministic test settlement.

    **Why this is a separate sub-project, not SP4.5.** SP4.5's charter is top-level panel engine-registration and the framework multi-window pixel-tallness bug. Resolving sub-view settlement requires: (i) deciding whether sub-views share the main scheduler (likely yes — matches C++) or get their own; (ii) changing settlement from "inside-Paint, synchronous" to "across-frames, scheduler-driven" (C++-faithful) or to "bounded synchronous drive-through `scheduler.do_time_slice` inside Paint" (preserves current timing); (iii) re-capturing golden baselines if single-frame snapshots change, or designing a test harness that drives settlement deterministically; (iv) rewriting the golden `settle()` helper. None of these are prerequisites for SP4.5's engine-registration work, and all of them belong in their own spec/plan.

    **Scope of SP8.**
    - Attach the main scheduler to `emSubViewPanel`'s inner `emView` at construction (or at parent-attach time), so sub-view panels can register `PanelCycleEngine` adapters with the shared scheduler.
    - Replace the `run_panel_cycles` / `HandleNotice` / `Update` / animator interleave in `SubViewPanel::Paint` with either (a) reliance on cross-frame scheduler ticks (C++-faithful; may need a Paint-side shim for first-frame correctness), or (b) a bounded synchronous drive-through `scheduler.do_time_slice` inside Paint (preserves current timing; keeps a Rust divergence of a different, narrower shape).
    - Delete `PanelTree::cycle_list`, `Cycle`, `cancel_cycle`, and `run_panel_cycles`.
    - Rewrite the golden `tests/golden/composition.rs::settle()` helper to drive settlement through whichever mechanism SP8 lands.

    **Blast radius estimate.** `emSubViewPanel` (~330 lines, core change); golden `composition.rs::settle` helper + ~4 call sites; `PanelTree` deletions; possibly one or more golden baselines if timing shifts. Likely bounded by the scheduler-attach shape chosen in scope item 1.

    **Dependencies and ordering.** No hard blockers from SP6 or SP7. Naturally pairs with SP7's `emContext` threading work — sharing `emContext` implies sharing scheduler in C++, and SP7 already plans to thread context through `emSubViewPanel` so the inner view inherits. Landing SP8 after SP7 is smaller; landing SP8 before SP7 is possible but will require a dedicated wiring mechanism that SP7 then subsumes.

    **Discovery trail.** SP4.5 Phase 4 grep (2026-04-19) for `run_panel_cycles` turned up two non-framework callers the original SP4.5 plan had not accounted for: `emSubViewPanel::Paint` (lines 303, 325) and the golden `settle()` helper. The user narrowed SP4.5 scope to drop the `PanelTree::cycle_list` / `run_panel_cycles` deletion step, leaving those APIs alive for SP8 to retire.
