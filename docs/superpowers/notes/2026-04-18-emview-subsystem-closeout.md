...
| Tests | 2434/2434 nextest, 9 skipped, 0 failed (2 new in SP5: `panel_data_view_defaults_none` field-default test, `sp5_per_view_notice_dispatch_uses_correct_pixel_tallness` multi-view dispatch test) |
...
| Known Rust-port incompletenesses remaining | SP4.5 (`emPanel` engine-registration port / `run_panel_cycles`), W3 surface de-dup (SP6, optional), `emContext` threading (SP7, ARCH). SP1 (animator-forward DIVERGED, re-entrancy docs, W4 polish), SP2 (`InvalidateHighlight` audit — landed in SP1 as W1b), SP3 (CoreConfig ownership), SP4 (`emView::Update` engine-only routing + Phase-8 single-engine test), and SP5 (per-view notice dispatch) all closed 2026-04-18. |
...
| **SP5 — Per-view notice dispatch** | ~~12~~ | **Complete 2026-04-18** (merged as `TODO(merge-sha)`; implemented across 16 commits on branch `sp5-per-view-notice-dispatch`). | `specs/2026-04-18-emview-sp5-per-view-notice-dispatch-design.md`, `plans/2026-04-18-emview-sp5-per-view-notice-dispatch.md` |
...
**Suggested execution order:** ~~SP1~~ → ~~SP3~~ → ~~SP4~~ → ~~SP5~~ → SP4.5 (naturally follows SP5 — same divergence shape, hard dep on SP5's `Rc<RefCell<emView>>` cascade) → SP6 if wanted → SP7 when the motivation arrives. (SP2 turned out to be already done — landed in SP1 as W1b.) SP1, SP3, SP4, and SP5 landed 2026-04-18.
...
12. ~~**[W5b / DEFERRED] Per-view notice dispatch (emView.cpp:1312 parity)**~~ **CLOSED 2026-04-18 by SP5** (branch `sp5-per-view-notice-dispatch`, TODO(merge-sha)). Resolution: `emView::HandleNotice` is now called from `emView::Update` per-view using that view's own `CurrentPixelTallness` and `window_focused` — emView.cpp:1312 parity. Ring data structure (`notice_ring_head_next`/`notice_ring_head_prev`) stays on `PanelTree` as a forced Rust divergence (RefCell re-entrancy prevents view-owned ring: the view is mutably borrowed during dispatch, so callbacks inside cannot re-borrow it to append). Annotated at `emPanelTree.rs:~297`. `emPanel::View: Weak<RefCell<emView>>` added (SP5 Phase 2) to establish per-panel view ownership. `emView` wrapped in `Rc<RefCell<emView>>` at `emWindow` and `emSubViewPanel` (SP5 Phase 1). SP4 test hacks deleted: `test_window_id` field on `emView`, `emWindow::new_for_test`, `emView::new_for_test` (SP5 Phase 5). Key commits (branch tip): `f53426a`, `9999eef`, `a0e882d`, `a64ab01`, `4d81961`, `a0ba42e`, `32028a6`, `1301a94`, `dbf8317`, `06535e5`, `8f67c0a`, `d7f4063`, `324942f`, `c8f8476`. Tests: 2432 → 2434 (+2: `panel_data_view_defaults_none`, `sp5_per_view_notice_dispatch_uses_correct_pixel_tallness`). `run_panel_cycles` is a separate Rust-only construct; stays open as SP4.5.
