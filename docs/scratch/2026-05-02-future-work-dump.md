...
## F019 post-implementation divergences

Surfaced during SDD execution of the F019 plan (`docs/superpowers/plans/2026-05-02-F019-emfilemodel-lazy-file-state-signal.md`); not present in the original brainstorm scratch. Branch `impl/f019` merged 2026-05-02.

- **F019 spec/plan are not retroactively updated.** Two material discoveries during execution diverged from the as-written spec:
  1. `dyn FileModelState` is used extensively in the codebase, so the spec's prescribed `GetFileStateSignal(&self, ectx: &mut impl SignalCtx) -> SignalId` trait method shape (generic, breaks dyn-compat) was replaced with a split: `GetFileStateSignal(&self) -> SignalId` stays as a non-allocating dyn-safe reader; a new `ensure_file_state_signal_dyn(&self, ectx: &mut dyn SignalCtx) -> SignalId` provides the lazy-allocator path. Concrete-arg form (callers pass `&mut EngineCtx<'_>` which coerces to `&mut dyn SignalCtx`).
  2. `emDirPanel` composes `emFilePanel` as a field rather than implementing `PanelBehavior`, so `<emFilePanel as PanelBehavior>::Cycle` (which holds B-015's connect block) never runs for derived panels. B-015's connect block was extracted into `emFilePanel::connect_file_state_signal` and called explicitly from `emDirPanel::Cycle`'s prefix.

  ISSUES.json `fix_note` (commit `6eb660de`) documents both with file:line citations. Spec at `docs/superpowers/specs/2026-05-02-F019-emfilemodel-lazy-file-state-signal-design.md` and plan at `docs/superpowers/plans/2026-05-02-F019-emfilemodel-lazy-file-state-signal.md` were left unchanged. Trigger to update: if/when the F019 pattern is referenced as an onboarding example for the dyn-safe-lazy-signal idiom or the composition-vs-PanelBehavior workaround, add a one-paragraph addendum at the bottom of each pointing to ISSUES.json for ground truth.

- **Sibling-panel B-015 unreachability sweep.** `emFileLinkPanel` and `emImageFileImageFilePanel` likely share `emDirPanel`'s composition-vs-PanelBehavior gap: both compose `emFilePanel` as a field, and B-015's connect block lives inside `<emFilePanel as PanelBehavior>::Cycle`, which never runs for them. Effect would be: even with F019's lazy file_state_signal landing, these derived panels still poll because the connect path is structurally unreachable — except their existing tests don't surface the issue (no equivalent of the silenced-model proof-of-fix test). Trigger: file as a follow-up bucket. Verification step: read each derived panel's `Cycle` for an explicit call to `emFilePanel::connect_file_state_signal`; if absent, lift it the same way `emDirPanel::Cycle` does (Task 2 of F019). Surface area: 2 panels + their tests + possible discovery that one or both already have an alternative subscribe path that obviates the lift.
