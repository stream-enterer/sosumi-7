...
pub(crate) fn sample_adaptive_premul_fp(
    ...
    for (col, col_slot) in col_y.iter_mut().enumerate() {
        ...
        for ch in 0..4 {
            col_slot[ch] = interpolate_four_values_adaptive(
                pm[0][ch], pm[1][ch], pm[2][ch], pm[3][ch], oy,
            );
        }
    ...
    for (ch, fp) in final_px.iter_mut().enumerate() {
        *fp = interpolate_four_values_adaptive(
            col_y[0][ch], col_y[1][ch], col_y[2][ch], col_y[3][ch], ox,
        );
    }
...
pub fn sample_adaptive_lum_section(
    ...
    let final_val = interpolate_four_values_adaptive(
        col_vals[0], col_vals[1], col_vals[2], col_vals[3], ox,
    );
