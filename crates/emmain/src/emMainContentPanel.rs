...
// ── Eagle polygon data (from C++ PaintEagle, emMainContentPanel.cpp:134-332) ─

static EAGLE_POLY0: [(f64, f64); 461] = [
...
static EAGLE_POLY4: [(f64, f64); 151] = [
...
static EAGLE_POLY10: [(f64, f64); 15] = [
...
static EAGLE_POLY11: [(f64, f64); 18] = [
...
static EAGLE_POLY12: [(f64, f64); 27] = [
...
static EAGLE_POLYS: [&[(f64, f64)]; 14] = [
    &EAGLE_POLY0, &EAGLE_POLY1, &EAGLE_POLY2, &EAGLE_POLY3, &EAGLE_POLY4,
    &EAGLE_POLY5, &EAGLE_POLY6, &EAGLE_POLY7, &EAGLE_POLY8, &EAGLE_POLY9,
    &EAGLE_POLY10, &EAGLE_POLY11, &EAGLE_POLY12, &EAGLE_POLY13,
];

static EAGLE_POLY_COLORS: [u32; 14] = [
...
impl emMainContentPanel {
    ...
    /// Render the eagle logo as 14 filled polygons.
    ///
    /// Port of C++ `emMainContentPanel::PaintEagle`.
    fn paint_eagle(&self, painter: &mut emPainter) {
        painter.push_state();
        painter.translate(self.eagle_shift_x, self.eagle_shift_y);
        painter.scale(self.eagle_scale_x, self.eagle_scale_y);

        for (poly, &color_packed) in EAGLE_POLYS.iter().zip(EAGLE_POLY_COLORS.iter()) {
            painter.PaintPolygon(poly, emColor::from_packed(color_packed), emColor::TRANSPARENT);
        }
...
mod tests {
    ...
    #[test]
    fn test_eagle_poly_sizes() {
        let expected: [usize; 14] = [461, 74, 6, 8, 151, 15, 71, 70, 18, 19, 15, 18, 27, 7];
        for (i, &size) in expected.iter().enumerate() {
            assert_eq!(EAGLE_POLYS[i].len(), size, "poly{i} size mismatch");
        }
    }

    #[test]
    fn test_eagle_poly0_first_vertex() {
        assert_eq!(EAGLE_POLY0[0], (79695.0, 46350.0));
    }

    #[test]
    fn test_eagle_poly0_last_vertex() {
        assert_eq!(EAGLE_POLY0[460], (77940.0, 46440.0));
    }
