...
static EAGLE_POLY_COLORS: [u32; 14] = [
    0x302030FF, 0x303040FF, 0x303040FF, 0x303040FF, 0x303040FF, 0x303040FF, 0x303040FF, 0x508080FF,
    0x505030FF, 0x505030FF, 0x505030FF, 0x508080FF, 0x505030FF, 0x000000FF,
];
    ...
    fn paint_eagle(&self, painter: &mut emPainter) {
        ...
        for (poly, &color_packed) in EAGLE_POLYS.iter().zip(EAGLE_POLY_COLORS.iter()) {
            painter.PaintPolygon(
                poly,
                emColor::from_packed(color_packed),
                emColor::TRANSPARENT,
            );
        }
